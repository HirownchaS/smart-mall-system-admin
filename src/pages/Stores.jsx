import React, { useState, useEffect, useRef } from "react";
import { Link, useNavigate } from "react-router-dom";
import {
  GridComponent,
  Inject,
  ColumnsDirective,
  ColumnDirective,
  Search,
  Page,
  Toolbar,
} from "@syncfusion/ej2-react-grids";
import Swal from "sweetalert2";
import jsPDF from "jspdf";
import "jspdf-autotable";
import Button from "@mui/material/Button";
import { Header } from "../components";

const Stores = () => {
  const [storesData, setStoresData] = useState([]);
  const [columns, setColumns] = useState([]);
  const [filteredData, setFilteredData] = useState([]);
  const gridRef = useRef(null);
  const navigate = useNavigate();

  const toolbarOptions = ["Search"];
  const editing = { allowDeleting: true, allowEditing: true };

  useEffect(() => {
    const fetchStores = async () => {
      try {
        const response = await fetch("http://localhost:8080/api/store");
        const data = await response.json();

        if (Array.isArray(data)) {
          setStoresData(data);

          const fieldsToDisplay = ["storeno", "name", "level", "category"];
          const cols = fieldsToDisplay.map((key) => ({
            field: key,
            headerText: key.charAt(0).toUpperCase() + key.slice(1),
            width: "150",
            textAlign: "Center",
          }));

          setColumns(cols);
        } else {
          console.error("Invalid data format from API");
        }
      } catch (error) {
        console.error("Error fetching stores:", error);
      }
    };

    fetchStores();
  }, []);

  const handleDataBound = () => {
    if (gridRef.current) {
      const filteredRecords = gridRef.current.getCurrentViewRecords();
      if (filteredRecords) {
        setFilteredData(filteredRecords);
      }
    }
  };

  const generatePDF = () => {
    const doc = new jsPDF();
    doc.text("Stores Report", 14, 10);
    doc.autoTable({
      head: [columns.map((col) => col.headerText)],
      body: filteredData.map((store) => columns.map((col) => store[col.field])),
    });
    doc.save("stores_report.pdf");
  };

  const editStore = (id) => {
    navigate(`/updateStore/${id}`);
  };

  const deleteStore = async (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      confirmButtonText: "Yes, delete it!",
    }).then(async (result) => {
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `http://localhost:8080/api/store/${id}`,
            {
              method: "DELETE",
            }
          );

          if (response.ok) {
            // If the store is deleted successfully
            console.log("Store deleted successfully");
            // Show success message
            Swal.fire({
              title: "Deleted!",
              text: "Your file has been deleted.",
              icon: "success",
            });
            // Navigate or refresh the store list
            window.location.reload(); // Assuming you want to go back to the list page
          } else {
            const result = await response.json();
            console.error("Error:", result.message);
            Swal.fire({
              title: "Error!",
              text:
                result.message || "An error occurred while deleting the store.",
              icon: "error",
            });
          }
        } catch (error) {
          console.error("Error:", error);
          Swal.fire({
            title: "Error!",
            text: "An error occurred while deleting the store.",
            icon: "error",
          });
        }
      }
    });
  };

  const actionTemplate = (rowData) => (
    <div className="flex justify-center gap-2">
      <Button
        variant="contained"
        color="primary"
        onClick={() => editStore(rowData._id)}
      >
        Edit
      </Button>
      <Button
        variant="contained"
        color="secondary"
        onClick={() => deleteStore(rowData._id)}
      >
        Delete
      </Button>
    </div>
  );

  return (
    <div className="m-2 md:m-10 mt-10 p-2 md:p-10 bg-white rounded-3xl">
      <div className="flex justify-between items-center">
        <Header category="Mall" title="Stores" />
        <div>
          <Button
            variant="contained"
            color="primary"
            style={{ marginRight: "10px" }}
          >
            <Link
              to="/addStore"
              style={{ color: "inherit", textDecoration: "none" }}
            >
              Add Store
            </Link>
          </Button>
          <Button variant="contained" color="secondary" onClick={generatePDF}>
            Generate Report
          </Button>
        </div>
      </div>

      {/* Conditionally render GridComponent only when storesData and columns are available */}
      {storesData.length > 0 && columns.length > 0 ? (
        <GridComponent
          dataSource={storesData}
          width="auto"
          allowPaging
          allowSorting
          pageSettings={{ pageCount: 5 }}
          editSettings={editing}
          toolbar={toolbarOptions}
          ref={gridRef}
          dataBound={handleDataBound}
        >
          <ColumnsDirective>
            {columns.map((col, index) => (
              <ColumnDirective key={index} {...col} />
            ))}
            <ColumnDirective
              headerText="Actions"
              template={actionTemplate}
              width="180"
              textAlign="Center"
            />
          </ColumnsDirective>
          <Inject services={[Search, Page, Toolbar]} />
        </GridComponent>
      ) : (
        <p>Loading stores data...</p>
      )}
    </div>
  );
};

export default Stores;
